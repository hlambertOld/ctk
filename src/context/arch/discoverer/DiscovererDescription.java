...
public class DiscovererDescription {
  ...
  public Error setHostname (DataObject data){
    ...
    if (data != null) {
      ...
      if (doHost != null){
        Vector vTemp = doHost.getValue();
        if ( ! vTemp.isEmpty()){
          setHostname((String) vTemp.firstElement());
          err.setError(Error.NO_ERROR);
          return err;
        }
      }
  ...
  Error err = new Error();
    if (data != null) {
      ...
      if (doPort != null){
        Vector vTemp = doPort.getValue();
        if ( ! vTemp.isEmpty()){
          setPort( new Integer((String)vTemp.firstElement()).intValue() );
          err.setError(Error.NO_ERROR);
          return err;
        }
      }
   ...
   */
    ...
    public synchronized void isAvailable(long time){
      if (! available){
        try {
          waiting = true;
          wait(time);
