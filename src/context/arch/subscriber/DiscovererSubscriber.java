...
public class DiscovererSubscriber extends AbstractSubscriber {
  ...
  /**
   * These fields are specific to Susbcriber 
   */
  ...
  public DiscovererSubscriber (DataObject data) {
    super(data);
    query = AbstractQueryItem.fromDataObject ((DataObject) (data.getDataObject (Discoverer.DISCOVERER_QUERY_CONTENT)).getChildren ().firstElement ());
    fullDescriptionResponse = Boolean.valueOf((String)data.getDataObject(Discoverer.DISCOVERER_DESCRIPTION_FULL_RESPONSE).getValue().firstElement()).booleanValue();
