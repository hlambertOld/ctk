...
public class CommunicationsObject {
  ...
  String serverClass, int serverPort, int poolClientsNumber) {
    ...
    // To be sure multicastClass is set to the default class (that could be
    // avoided after...)
    if(multicastClass == null)
      this.multicastClass = this.DEFAULT_MULTICAST_CLIENT_SERVER;
  ...
  public void start() throws InvalidProtocolException {
    if (System.getProperty("os.name").equals(Constants.WINCE)) {
      ...
      try {
        ...
        Class[] classesMulticast = new Class[1];
        classesMulticast[0] = Class.forName("context.arch.comm.CommunicationsObject");
        //classesMulticast[1] = Class.forName("java.lang.Integer");
        Constructor constructorMulticast = Class.forName(multicastClass).getConstructor(classesMulticast);
        Object[] objectsMulticast = new Object[1];
        objectsMulticast[0] = this;
        multicastCS = (CommunicationsMulticast) constructorMulticast.newInstance(objectsMulticast);
  ...
  public void handleIncomingRequest(DatagramPacket packet) {
    ...
    if (error == null){
      ...
      componentHost = null;
      int componentPort = this.DEFAULT_PORT;
      ...
      try {
        ...
        componentName = (String) (componentAdd.getDataObject(Discoverer.ID)).getValue().firstElement();
        componentHost = (String) (componentAdd.getDataObject(Discoverer.HOSTNAME)).getValue().firstElement();
        Integer intPort = new Integer((String)(componentAdd.getDataObject(Discoverer.PORT)).getValue().firstElement());
        componentPort = intPort.intValue();
        
        //Create another DataRequest to reply
        Vector vContent = results_temp.getChildren();
        DataObject doContent = (DataObject) vContent.elementAt(1);
        Vector contentToSend = doContent.getChildren();
      ...
      try{
        ...
        RequestData descriptionComponent = sendRequest(request);
